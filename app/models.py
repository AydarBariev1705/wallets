...
class WalletModel(Base):
    ...
    deposits = relationship('DepositModel', back_populates='wallet', cascade='all, delete-orphan')
    withdraws = relationship('WithdrawModel', back_populates='wallet', cascade='all, delete-orphan')

    def __init__(self, balance=0.0):
        self.id = uuid4()
        self.balance = max(balance, 0.0)  # Гарантирует, что баланс всегда >= 0
        self.deposits = []  # Инициализация пустого списка депозитов
        self.withdraws = []  # Инициализация пустого списка выводов средств
...
class DepositModel(Base):
    ...
    wallet = relationship('WalletModel', back_populates='deposits')
...
class WithdrawModel(Base):
    ...
    wallet_id = Column(UUID(as_uuid=True), ForeignKey('wallets.id'), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    wallet = relationship('WalletModel', back_populates='withdraws')
